...
class BayesActorCriticAgent(PolicyGradientAgent):
    ...
    def initialize_models(self) -> None:
        ...
        if self.config.optimizer == "Adam":
            self.attacker_optimizer = torch.optim.Adam(self.attacker_policy_network.parameters(),
                                                       lr=self.config.alpha_attacker)
            self.defender_optimizer = torch.optim.Adam(self.defender_policy_network.parameters(),
                                                       lr=self.config.alpha_defender)
        elif self.config.optimizer == "SGD":
            self.attacker_optimizer = torch.optim.SGD(self.attacker_policy_network.parameters(),
                                                      lr=self.config.alpha_attacker)
            self.defender_optimizer = torch.optim.SGD(self.defender_policy_network.parameters(),
                                                      lr=self.config.alpha_defender)
        else:
...
.
    ...
    def training_step(self, saved_rewards: List[List[float]], saved_log_probs: List[List[torch.Tensor]],
        ...
        for batch in range(num_batches):
            ...
            ##### Bayesian Updates
            a, Qall = self.GPTD_step(saved_state_values[batch], saved_log_probs[batch])


            a = a.to(torch.device("cuda:" + str(self.config.gpu_id)))
            Qall = Qall.to(torch.device("cuda:" + str(self.config.gpu_id)))

            print(a.shape)

            # Compute PG "loss" which in reality is the expected reward, which we want to maximize with gradient ascent
            for log_prob, state_value, R, a0, q in zip(saved_log_probs[batch], saved_state_values[batch], returns, a[0,:], Qall[0,:]):
                ...
                policy_loss.append(-log_prob * a0)
    ...
    def get_action(self, state: np.ndarray, attacker: bool = True, opponent_pool=False,
                   legal_actions: List = None, non_legal_actions: List = None) -> Union[
        int, torch.Tensor, torch.Tensor, np.ndarray]:
        """
    ...
    def train(self) -> ExperimentResult:
        ...
        for iter in range(self.config.num_episodes):
            ...
            if iter % self.config.train_log_frequency == 0:
                ...
                self.log_metrics(iter, self.train_result, episode_attacker_rewards, episode_defender_rewards,
                                 episode_steps,
            ...
            while not done:
                ...
                if self.config.attacker:
                    ...
                    attacker_action, _, _, _ = self.get_action(attacker_state, attacker=True,
                                                               ...
                                                               non_legal_actions=illegal_actions)
                    if self.env.local_view_features():
                        attacker_action = PolicyGradientAgent.convert_local_attacker_action_to_global(attacker_action,
                                                                                                      attacker_obs)
                if self.config.defender:
    ...
    def GPTD_step(self, nu = 0.1, M = 100):
        ...
        for m in range(M):
            ...
            for t in range(len(grad_dict)):
                ...
                k[t, 0] = torch.exp(-torch.linalg.norm(uzi - uzj) / (2*100))
            ...
            at = Kprev_inv @ k
            ktt = torch.exp(-torch.linalg.norm(uzj - uzj) / (2*100))
            delta = ktt - k.T @ at

            Dkt = kprev - gamma*k

            dt = (gamma*sig2prev/stprev)*dtprev + rprev - Dk.T @ alpha_prev
            ...
            if delta > nu:
                ...
                ht = torch.cat((atprev, -gamma),1)
                Dktt = atprev.T @ (kprev - 2*gamma*k) + (gamma**2)*ktt

                ct = (gamma*sig2prev/sprev)*torch.cat((ctprev, 0),0) + ht - torch.cat((Ctprev @ Dkt, 0),0)

                st = sig2prev + (gamma**2)*sig2 + Dktt - Dkt.T @ Ctprev @ Dkt + 2*(gamma*sig2prev/sprev)*ctprev.T @ Dkt - (((gamma*sig2prev)**2)/sprev)

                alpha_prev = torch.cat((alpha_prev, 0), 0)

                zero_fill = torch.zeros_like(Ctprev[:, 0]).reshape(-1, 1)
                Ctprev = torch.hstack([torch.vstack([Ctprev, zero_fill.T]), torch.vstack([zero_fill, 0])])

            else:

                ht = atprev - gamma*at
                Dktt = ht.T @ Dkt
                ct = (gamma*sig2prev/sprev)*ct + ht - Ctprev @ Dkt
                st = sig2prev + (gamma**2)*sig2 + Dkt.T @ (ct + (gamma*sig2prev/sprev)*ctprev) - (((gamma*sig2prev)**2)/sprev)    + Dktt #????


            alpha_t = alpha_prev + (ct/st)*dt
            Ct = Ctprev + (1/st)* ct @ ct.T

            return dict, alpha_t, Ct
