...
class BayesReinforceAgent(PolicyGradientAgent):
    ...
    def training_step(self, R, Post_mean, lr, Fisher_inv, log_probs, attacker=True):
        ...
        for batch in range(num_batches):
            ...
            returns = torch.Tensor([0]*R.shape[0])
        ...
        for iter in range(self.config.num_episodes):
            ...
            for m in range(M):
                ...
                if flag0:
                    ...
                    Z = Variable((1 + uz1.T @ Ginv @ uz1), requires_grad=True)
                    #Y = Variable(torch.Tensor([R[-1]]), requires_grad=True)
                ...
                Post_mean, Kinv, flag, Y, Z = self.Kernel_Online_vs2(last_grad=saved_grads, grad_dict=saved_grads_batch,
                                                                      Ginv=Ginv, R=R, Kprev=Kinv,
                                                                      episode=m, max_episode=M - 1, nu=nu0, yM=Y, zM=Z)
            ...
            if self.config.attacker:
                ...
                loss = self.training_step(Post_mean=Post_mean, R=R, attacker=True, lr=lr_attacker, Fisher_inv=Ginv, log_probs=saved_attacker_log_probs_batch)
            ...
            if self.config.defender:
                ...
                loss = self.training_step(Post_mean=Post_mean, R=R, attacker=True, lr=lr_attacker, Fisher_inv=Ginv)
    ...
    def Var_params(self):
        ...
        for param in model.parameters():
            W = torch.hstack((W, param.flatten()))
        W = W.reshape(-1,1)
        Var = W @ W.T
        return Var
    ...
    def Kernel_Online_vs2(self, last_grad, grad_dict, Ginv, R, yM, zM, Kprev = None, sigma2 = 100, nu = 0.0001, model_type = 1, episode = None, max_episode = None):
        ...
        uzt = torch.stack(last_grad, dim=0).sum(dim=0).reshape(-1, 1)
        ktt = torch.pow(1 + uzt.T @ Ginv @ uzt, 2)
        ...
        if delta > nu:
            ...
            yM = torch.cat((yM, (R[-1] * uzj)), 1)
            zM = torch.cat((zM, (1 + uzj.T @ Ginv @ uzj)), 1)

            #aux = torch.Tensor([R[-1]]).reshape(1, -1)
            #if torch.cuda.is_available() and self.config.gpu:
            #    device = torch.device("cuda:" + str(self.config.gpu_id))
            #    aux = aux.to(device)
            #yM = torch.cat((yM.reshape(1, -1), aux), 1)
    ...
    def parametric_MC_GPTD(self, gradprev, gradpost,
                           wt_prev, Pt_prev, pt_prev, stprevinv,
                           r_prev, sigma2prev = 1, sigma2post = 1):

        delta_phi = gradprev - self.config.gamma*gradpost

        pt = (self.config.gamma*sigma2prev*stprevinv)*pt_prev \
             + Pt_prev @ delta_phi

        dt = (self.config.gamma*sigma2prev*stprevinv)*dt_prev \
             + r_prev \
             - delta_phi.T @ wt_prev

        st = sigma2prev \
             + (self.config.gamma**2)*sigma2post \
             - (((self.config.gamma*sigma2prev)**2)*sprevinv) \
             + (pt + (self.config.gamma*sigma2*stprevinv)*pt_prev).T @ delta_phi
