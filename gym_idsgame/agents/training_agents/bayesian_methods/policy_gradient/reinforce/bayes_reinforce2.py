...
class BayesReinforceAgent(PolicyGradientAgent):
    ...
    def training_step(self, X, logP, R, Ginv, Cinv, model_type=1,attacker=True):
        ...
        for batch in range(num_batches):
            ...
            '''
            #R = (R - torch.mean(R)) / (torch.std(R))
            #print(torch.mean(R))
            #print('')
            #print(R.shape[0])
            #print( (R - torch.mean(R)) / (torch.std(R)*(R.shape[0] > 1) + self.machine_eps) )
            ...
            if R.shape[0] > 1:
                ...
                #R = (R) / (torch.std(R) + self.machine_eps)
            #print(R)
            #print(returns)
    ...
    def train(self) -> ExperimentResult:
        ...
        print(start1.elapsed_time(end1))  # milliseconds
        '''
        ...
        for iter in range(self.config.num_episodes):
            ...
            for m in range(M):
                ...
                end1.record()
                torch.cuda.synchronize()
                print('sampling episodes', start1.elapsed_time(end1))  # milliseconds
                ...
                startko = torch.cuda.Event(enable_timing=True)
                endko = torch.cuda.Event(enable_timing=True)

                startko.record()

                Cinv, Kinv, flag = self.Kernel_Online(path_dict=X, Ginv=Ginv, Kprev=Kinvprev, episode=m,
                                                      max_episode=M - 1, nu=nu0)
                Kinvprev = Kinv

                endko.record()
                torch.cuda.synchronize()
                print('Time for Kernel_Online', startko.elapsed_time(endko))  # milliseconds
            ...
            startts = torch.cuda.Event(enable_timing=True)
            endts = torch.cuda.Event(enable_timing=True)
            startts.record()
            ...
            endts.record()
            torch.cuda.synchronize()
            print('Time for Training Step', startts.elapsed_time(endts))  # milliseconds
    ...
    def Kernel_Online(self, path_dict, Ginv, Kprev = None, sigma2 = 100, nu = 0., model_type = 1, episode = None, max_episode = None):
        if Kprev == None:
