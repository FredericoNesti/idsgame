...
class BayesReinforceAgent(PolicyGradientAgent):
    ...
    """
    def __init__(self, M:int, risk_averse: bool, nu: float, static: bool, env:IdsGameEnv, config: PolicyGradientAgentConfig):
        """
    ...
    def get_action(self, state: np.ndarray, attacker : bool = True, legal_actions : List = None,
        ...
        try:
            ...
            if self.config.attacker:
                ...
                if self.risk_averse:
                    #action = Variable(torch.argmin(state).type(torch.LongTensor), requires_grad=True)
                    action = torch.argmin(state).type(torch.LongTensor)
                if self.static:
                    #action = Variable(torch.tensor(3).type(torch.LongTensor), requires_grad=True)
                    action = torch.tensor(3).type(torch.LongTensor)
                if not self.static and not self.risk_averse:
                    action = policy_dist.sample()
        ...
        if attacker:
            ...
            for param in self.defender_policy_network.parameters():
        ...
        if attacker:
            ...
            with torch.no_grad():
                ...
                for param in self.defender_policy_network.parameters():
                    ...
                    i += aux
                    tochange += self.config.alpha_defender * prepare_grad
    ...
    def train(self) -> ExperimentResult:
        ...
        for iter in range(self.config.num_episodes):
            ...
            #add to tensorboard delta threshold
            #if self.config.attacker:
            #    self.tensorboard_writer.add_scalar('delta_threshold/train/attacker', nu0, iter)
            #if self.config.defender:
            #    self.tensorboard_writer.add_scalar('delta_threshold/train/defender', nu0, iter)
            ...
            for episode in range(self.config.batch_size):
                ...
                for m in range(M):
                    ...
                    if flag0:
                        ...
                        if self.config.attacker:
                            ...
                            R.append(episode_defender_reward)
                    ...
                    if m > 0:
                        ...
                        Post_mean, K, Kinv, flag, Y, Z, A, Cinv, record_deltas = self.Kernel_Online_vs2(last_grad=saved_grads, grad_dict=saved_grads_batch,
                                                                              Ginv=Ginv, R=R, Kprev=K, Kprev_inv=Kinv, A = A, C_inv = Cinv, sig2=sig2,
                                                                              episode=m, max_episode=M - 1, nu=nu0, yM=Y, zM=Z, one=one, iter=iter, batch=episode,
                                                                                         deltas=record_deltas)
                ...
                #summary_heatmap("train/kernel_heatmap", K)
                #self.tensorboard_writer.add_summary(summary_heatmap, global_step=iter*episode + episode)
                #self.tensorboard_writer.add_image(str(train_episode) + "_eval_frames/" + str(idx),frame, global_step=train_episode,dataformats="HWC")
            ...
            if self.config.defender:
                #loss = self.training_step(saved_attacker_rewards_batch, saved_attacker_log_probs_batch, attacker=True)
                loss = self.training_step(Post_mean=post_mean_batch, R=saved_defender_rewards_batch, attacker=True, lr=lr_defender, Fisher_inv=Ginv, log_probs=saved_defender_log_probs_batch)
                episode_attacker_loss += loss.item()
    ...
    def Kernel_Online_vs2(self, last_grad=None, grad_dict=None,
        ...
        if self.config.defender:
            self.tensorboard_writer.add_scalar('delta/train/defender', delta, self.config.batch_size*iter*max_episode + episode + batch)
        ...
        return Post_mean, K, Kinv, flag_dictionary, yM, zM, A, C_inv, deltas
