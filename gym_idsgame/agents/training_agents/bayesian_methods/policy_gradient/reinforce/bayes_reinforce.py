...
class BayesReinforceAgent(PolicyGradientAgent):
    ...
    '''
    def score_vector(self, a, mx, sx):
        u = Variable(torch.zeros(2,1), requires_grad=True)
    ...
    '''
    def Fisher_Info(self, X, A):
        d = 0
        Fisherinfo = Variable(torch.zeros(2,2), requires_grad=True)
        if torch.cuda.is_available() and self.config.gpu:
            device = torch.device("cuda:" + str(self.config.gpu_id))
        ...
        for x,a in zip(X,A):
            ...
            mx = torch.mean(x)
            sx = torch.var(x)
            uz = self.score_vector(a, mx, sx)
            Fisherinfo += uz @ uz.T
        Fisherinfo /= d
        return Fisherinfo

    def Fisher_Info_vs2(self, State):
        d = 0
        flag = False
        #print('')
        #print('INSIDE Fisher INfo vs2')
        ...
        for x in State:
            ...
            #print(x.shape)
            if flag == False:
                fisherinfo = - hessian(func=self.log_prob_func_attacker, inputs=x)
                #print('')
                #print(fisherinfo)
            if flag == True:
                fisherinfo += - hessian(func=self.log_prob_func_attacker, inputs=x)
    ...
    def Kernel_func(self, X, A, Ginv, model_type = 1):
        K = Variable(torch.eye(len(X)), requires_grad=True)
        if torch.cuda.is_available() and self.config.gpu:
            device = torch.device("cuda:" + str(self.config.gpu_id))
            K = K.to(device)
        #print('ok0')
        ...
        for i in range(K.shape[0]):
            ...
            for j in range(K.shape[1]):
                ...
                #uzj = self.score_vector(a = A[i], mx=torch.mean(X[j]), sx=torch.var(X[j]))
                uzi = self.score_func(state = X[i])
                uzj = self.score_func(state = X[j])
                #if model_type == 1:
                K[i,j] += torch.pow(1 + uzi.T @ Ginv @ uzj, 2) * (model_type == 1) + uzi.T @ Ginv @ uzj * (model_type == 2)
                #if model_type == 2:
    ...
    def Kernel_Online(self, path_dict, Ginv, Kprev = None, sigma2 = 100, nu = 0.00001, model_type = 1, episode = None, max_episode = None):
        if Kprev == None:
